...
def character_entry_strings(request):
    return {'label': f'Entry {request.param}', 'value': f'{request.param}'}


@pytest.fixture(scope='module', params=entry_value_numbers)
def character_entry_numbers(request):
    return {'label': f'Entry {request.param}', 'value': int(request.param)}
...
class TestCharacterEntry:
    ...
    def test_string_entry(self, character_entry_strings):
        ...
        assert entry.label == character_entry_strings.get('label')
        assert entry.name == slugify(entry._label)
        assert entry.value == character_entry_strings.get('value')
    ...
    def test_number_entry(self, character_entry_numbers):
        ...
        assert entry.label == character_entry_numbers.get('label')
        assert entry.name == slugify(entry._label)
        assert entry.value == character_entry_numbers.get('value')
